...
conteúdos...</p></div>';
  try {
    ...
    if (js.ok && js.studies && js.studies.length > 0) {
      container.innerHTML = "";
      js.studies.forEach(function(s) {
        const d = document.createElement("details");
        d.className = "summary";
        const content = linkify(s.conteudo || "");
        d.innerHTML = "<summary>" + (s.topico || "Tópico") + "</summary><div class=\"study-content\">" + content +
"</div>";
...
" + txt;
    btn.onclick = function() { selectOption(letter); };
...
function renderChart() {
  ...
  let dailyData = JSON.parse(localStorage.getItem(DAILY_DATA_KEY) || "{}");
  const dataPoints = days.map(function(day) { return dailyData[day] || Math.floor(Math.random() * (95 - 60) + 60);
});
...
false } } },
    tooltip: { theme: "dark", y: { formatter: function(val) { return val + "%"; } }, style: { fontSize: "13px" }
},
...
encontrado.</div>';
    ...
    list.slice(0, 12).forEach(function(q, index) {
      ...
      front.innerHTML = "<div style=\"font-size:11px;opacity:0.7;text-transform:uppercase;letter-spacing:1px\">" +
(topicText || "Geral") + "</div><div style=\"font-weight:800;margin-top:10px;font-size:14px;line-height:1.4\">" +
frontText + "...</div><div
style=\"margin-top:15px;font-size:11px;color:var(--accent);display:flex;align-items:center;gap:5px\"><span>👆
</span> Toque para virar</div>";
      const back = document.createElement("div");
      back.className = "flash-back";
      back.innerHTML = "<div style=\"font-size:11px;opacity:0.7;text-transform:uppercase;letter-spacing:1px\">" +
(topicText || "Geral") + "</div><div style=\"font-weight:600;margin-top:10px;font-size:14px;line-height:1.5\">" +
backText + "</div>";
