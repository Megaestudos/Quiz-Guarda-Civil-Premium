...
def fix_file(filepath):
    try:
        ...
        # Regex to remove all <style>...</style> blocks completely
        pattern = re.compile(r'<style>.*?</style>', re.DOTALL | re.IGNORECASE)
        content_no_styles = pattern.sub('', content)
        
        # Insert the single style block right before </head>
        replacement = f'<style>\n{CSS_CONTENT}\n    </style>\n</head>'
        new_content = content_no_styles.replace('</head>', replacement, 1)
